...
- **The severity colours are lifted off the spec's literal values too**, for the same reason and found the same way. `#7fa187` / `#b07a3c` / `#a65a52` measured 4.8 / 3.7 / **2.7**:1 against a raised card — so the higher the severity, the dimmer it rendered, and a 9 or 10 came out as the least legible text on screen, below the AA floor, on the one line the timeline exists to be scanned for. The current values measure 5.8 / 4.9 / 4.7:1 on a card and 7.3 / 6.3 / 6.0:1 on the page. **Check any new severity colour against `bg-raised`, not just against the page** — the card is the tighter of the two.
- **`--color-text-primary` is deliberately *not* the spec's literal `#e4dfd6`.** That measured 13.1:1 against the page and 10.3:1 against a raised card — close to max contrast, and it read as glare on device. The spec asks for AA (4.5:1) with **both** ends pulled inward, so the literal value worked against its own rule. `#cdc7bb` measures 10.3:1 and 8.1:1, still past AAA (7:1) for body text. Don't restore `#e4dfd6` as a spec correction.
