...
**Every block is an `InsightSection`** — heading, content and the note explaining it inside one `bg-bg-surface` card, so a caption belongs visibly to the chart above it instead of floating between two blocks.
...
- **Ending an attack also asks how much it stopped you doing things** — `Attack.impact`, 0 none / 1 a little / 2 a lot / 3 couldn't function. Functional impairment is the disability measure the clinical scales (MIDAS, HIT-6) are built on and the number that most changes how a headache history is read, and it cannot be derived from anything else the app collects. It's asked **when the attack ends**, not when it starts: it's a judgement about the whole episode, nobody knows the answer in the first ten minutes, and it's the one place it costs no extra step in an eight-screen wizard. **Skipping leaves it `undefined`, never 0** — "not answered" and "no impact" are different facts and only one belongs in a disability count, which is why `endAttack` omits the key entirely rather than writing a default, and why `sync.ts` maps `null` back to absent. Needs `alter table attacks add column if not exists impact smallint;` on an existing Supabase table.
