...
**Every block is an `InsightSection`** — heading, content and the note explaining it inside one `bg-bg-surface` card, so a caption belongs visibly to the chart above it instead of floating between two blocks. That puts **three tones** in play: the page (`bg-base`), the section (`bg-surface`), and whatever the section nests inside itself (`bg-raised`). Content placed in a section must use `bg-bg-raised` — `bg-bg-surface` disappears into its own container. The stat tiles at the top are the exception: they're already cards, and wrapping cards in a card says nothing.
