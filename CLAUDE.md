...
**"Nothing changed" is a reminder's job, not a screen's.** It stays a quick action on the notification, where it's answerable from a lock screen or a watch without unlocking anything — which is the only place it saves work. `App.tsx`'s `handleNoChange` went with the choice screen; a `no_change` reading now only ever arrives through the notification queue drain (`consumePendingActions`), which is what the native handler was built for.
