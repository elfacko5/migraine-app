...
**"It's over — end attack" survives, as a quiet tertiary action on step 1.** It was the one thing the choice screen offered that nothing else in this sheet does, and dropping it would send someone back out to the Today card to end an attack they're already looking at — the detour it was added to remove. It still opens `EndAttackDialog` rather than ending immediately, so the time presets and the minute-vs-second clamping aren't duplicated, and confirming closes the update sheet underneath it.
