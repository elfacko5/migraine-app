...
- **Text scale runs 87.5%–150%.** This is a deliberate divergence from the dossier, which asks for 200% per WCAG 1.4.4 — it shipped at 200% and came back "ridiculous" on device, four words to a line on an app that has to be usable mid-attack. The chrome size caps that 200% testing exposed are still in place. **Revisit only with the user; it isn't an oversight.** For reference, the steps are `[data-scale]` on `<html>`: 14 / 15 / 16 / 22 / 24px. **`BottomNav` and `AttackModePill` cap their own font and icon sizes** (`min(0.875rem, 16px)` and friends); that was found at 200%, where the unclamped nav pushed Insights into a clip and Profile off screen entirely — a whole tab unreachable — and it costs nothing at 150%. Anything added to the persistent chrome needs the same treatment.
