...
- **Notification action buttons have not been round-tripped on a device.** Synthetic taps on Simulator notifications don't activate them — confirmed again while debugging the "Something changed" failure, so a tap that produces no log line there proves nothing. What *has* been checked on the simulator: the app launches and runs normally with Capacitor's notification delegate disabled and `NotificationActionHandler` installed in its place; the handler really is the delegate and really is dispatched to (`willPresent` fires); `capacitorRouter()` and its `localNotificationHandler` are both live by `applicationDidBecomeActive`; and a reminder carrying the `followUpMs` payload is accepted by the OS (`Added notification request: hasError: 0`). What a device would settle: that `didReceive` is reached for a background action after a force-quit, and that the follow-up reminder it schedules fires. Everything after the queue write — reading application with the tap time preserved, several queued answers in one commit, the wizard opening from an `update` entry, and the ended/deleted-attack guards — is verified on the web build and is the same code on both platforms.
